...
export function loadFromResource(nova:NovaNotePlugin,resourceName:string,on:OPR_TYPE):BlockData{
...
export const processConditions = (data:BlockDataElm,curData:FileData,conditions:OPR_TYPE):boolean=>!!processOPR(data,curData,conditions);
export function processOPR(data:BlockDataElm,curData:FileData,conditions:OPR_TYPE){
    ...
    function processCondition(condition:OPR_TYPE):VAL_TYPE{
        if(isOfValType(condition)){
            ...
            let value:any = processDataLocation(props,data,curData);
...
function processDataLocation(props:string[],blockData:BlockDataElm,curData:FileData){
    switch(props[0]){
        case '$file':
            splice();
