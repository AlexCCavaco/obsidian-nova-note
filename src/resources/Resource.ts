...
import { loadFromResource, processOPR, type FileData } from "src/blocks/dataLoader";
...
export default class {
    ...
    assert(nova:NovaNotePlugin,data:BlockDataElm,curData:FileData):BlockDataElm{
        ...
        for(const key in this.cols){
            ...
            else if(col.type==='resource') nData[key] =  { lazy:true,get:()=>{nData[key]=col.resource?loadFromResource(nova,col.resource,col.on,nData):def;return nData[key];} };
            else if(col.type==='value') nData[key] =  { lazy:true,get:()=>{nData[key]=processOPR(data,curData,col.value);return nData[key]??def;} };
        }
