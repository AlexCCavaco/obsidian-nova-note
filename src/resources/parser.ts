...
export type RESOURCE_COL_TYPE = {
    label:string,
    type:'text'|'number'|'select'|'multisel'|'list'|'check'|'file'|'folder'|'url'|'date'|'time'|'datetime'|string,
...
export const parser = seqMap(
    SWORD,
    regex(/TEXT|NUMBER|SELECT|MULTISEL|LIST|CHECK|FILE|FOLDER|URL|DATE|TIME|DATETIME/i).skip(W_EOF).map(type=>([type.toLowerCase()]))
